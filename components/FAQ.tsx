...
export default function FAQ() {
  const itemClasses = {
    ...
    heading: "p-4 md:p-5",
    title:
      "font-Montserrat text-xl md:text-3xl text-secondary focus-within:outline-none focus-within:border-transparent focus-within:ring-0 font-bold",
    trigger: "h-auto lg:h-10",
    indicator: "text-xl md:text-2xl text-white",
    content:
      "p-4 md:p-5 font-Lato text-sm md:text-base text-primary bg-white font-medium",
  };
  return (
    <section className="padding-variable" id="FAQ">
    ...
    <Accordion variant="splitted" itemClasses={itemClasses}>
      ...
      isOpen ? (
        <AiOutlineMinus className="h-5 w-5 text-secondary rotate-90" />
      ) : (
        <AiOutlinePlus className="h-5 w-5 text-secondary" />
      )
    } title={item.title} className="bg-gradient font-medium">
        {/* {item.content} */}
